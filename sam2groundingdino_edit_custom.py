...
from EditAnything.annotator.util import resize_image, HWC3
...
from EditAnything.utils.stable_diffusion_controlnet_inpaint import StableDiffusionControlNetInpaintPipeline
import torch
import datetime
import os
#시간
...
sam_checkpoint = "./EditAnything/models/sam_vit_h_4b8939.pth"
groundingdino_checkpoint = "./EditAnything/models/groundingdino_swint_ogc.pth"
groundingdino_config_file = "./EditAnything/GroundingDINO_SwinT_OGC.py"
model_type = "default"
...
if not use_gradio:
    image_path = "./EditAnything/assets/me.jpg"
    input_image_pil = Image.open(image_path).convert('RGB')
    input_image = np.array(input_image_pil, dtype=np.uint8)[:, :, :3]

    mask_prompt = 'person.'
    # mask_image = np.array(prompt2mask(input_image, mask_prompt), dtype=np.uint8)
    prompt = "iron man from the marvel cinematic universe."
    a_prompt = 'high quality, detailed, realistic'
    ...
    save_image(image_list, f"{output_dir}result.jpg", nrow=3,
